...
async def test_admin_lists_prompt_versions_with_active_flags(tmp_path: Path) -> None:
    ...
    with engine.begin() as connection:
        ...
        _insert_prompt_template(
            ...
            version=6,
            content="inactive llm1_system v6",
            is_active=False,
        )
    ...
    assert payload["items"][0] == {"name": "llm1_system", "version": 6, "is_active": False}
    assert {"name": "llm1_system", "version": 5, "is_active": True} in payload["items"]
...
async def test_admin_gets_active_prompt_version_by_name(tmp_path: Path) -> None:
    ...
    assert response.status_code == 200
    assert response.json() == {"name": "llm1_system", "version": 5, "is_active": True}
...
async def test_admin_renders_prompt_management_html_page_with_versions(tmp_path: Path) -> None:
    ...
    assert 'href="/admin/prompts/llm1_system/versions/6"' in response.text
...
async def test_admin_renders_prompt_version_content_page(tmp_path: Path) -> None:
    ...
    with engine.begin() as connection:
        ...
        _insert_prompt_template(
            ...
            version=6,
            content="PROMPT V6 CONTENT",
            is_active=False,
        )

    with _build_client(async_url, token_service=token_service) as client:
        response = client.get(
            "/admin/prompts/llm1_user/versions/6",
            headers={"Authorization": f"Bearer {admin_token}"},
        )
    ...
    assert "PROMPT V6 CONTENT" in response.text
