...
- [x] 6.3 Update this `tasks.md` with completed checkboxes and verification notes as implementation slices land.
...
- Slice 6.2 verification executed successfully:
  ...
  - Tentativa adicional de rodar `uv run mypy` também sobre os testes alterados falhou em problemas de tipagem existentes em `tests/unit/test_llm1_validation.py`, `tests/integration/test_process_pdf_case_llm1.py` e `tests/integration/test_worker_runtime_service_wiring.py`; por isso o registro aplicável do slice ficou limitado aos caminhos Python de código-fonte alterados.
- Slice 6.3 verification executed successfully:
  - `markdownlint-cli2 "openspec/changes/eda-decision-rulebook-rewrite/tasks.md"`
