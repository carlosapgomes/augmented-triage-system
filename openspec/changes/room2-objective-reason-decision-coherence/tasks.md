...
- [ ] 1.2 Adicionar/ajustar testes unitários para validar derivação determinística das causas de negativa na ordem: exclusão EDA, pendência laboratorial obrigatória, ECG obrigatório ausente, fallback de segurança.
...
- [ ] 2.1 Refatorar `src/triage_automation/infrastructure/matrix/message_templates.py` para bifurcar construção de `Motivo objetivo` por decisão final (`deny` vs `accept`).
